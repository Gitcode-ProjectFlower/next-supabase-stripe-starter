...
export class IdempotencyHandler {
  ...
  static async checkIdempotency(idempotencyKey: string, userId: string): Promise<{ exists: boolean; result?: any }> {
  ...
  static async storeResult(idempotencyKey: string, userId: string, result: any): Promise<void> {
    const key = `idempotency:${userId}:${idempotencyKey}`;
...
export function getRequestId(request: Request): string {
  return request.headers.get('x-request-id') || IdempotencyHandler.generateRequestId();
}
