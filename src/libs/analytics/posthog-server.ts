...
export const trackServerEvent = {
  // Checkout events
  checkoutStarted: (params: { userId: string; planName: string; price: number; interval: string }) => {
  ...
  subscriptionCreated: (params: { userId: string; planName: string; price: number; interval: string }) => {
    posthogServer.capture({
      distinctId: params.userId,
  ...
  subscriptionUpdated: (params: { userId: string; planName: string; price: number; interval: string }) => {
    posthogServer.capture({
      distinctId: params.userId,
  ...
  selectionCreated: (params: { userId: string; selectionId: string; itemCount: number; hasFilters: boolean }) => {
    posthogServer.capture({
      distinctId: params.userId,
  ...
  qaCompleted: (params: { userId: string; selectionId: string; itemsCount: number; duration: number }) => {
    posthogServer.capture({
      distinctId: params.userId,
      event: 'qa_completed',
      properties: {
