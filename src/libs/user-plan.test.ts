...
import { getPlanCap, getVisibleColumns, maskFields } from './user-plan';

describe('user-plan', () => {
  describe('getPlanCap', () => {
    it('should return 300 for small plan', () => {
      expect(getPlanCap('small')).toBe(300);
    });

    it('should return 2000 for medium plan', () => {
      expect(getPlanCap('medium')).toBe(2000);
    });

    it('should return 8000 for large plan', () => {
      expect(getPlanCap('large')).toBe(8000);
    });

    it('should return 3 for null plan (treated as anonymous)', () => {
      expect(getPlanCap(null)).toBe(3);
    });

    it('should return 100 for free_tier plan', () => {
      expect(getPlanCap('free_tier')).toBe(100);
    });

    it('should return 3 for anonymous plan', () => {
      expect(getPlanCap('anonymous')).toBe(3);
    });
  });
