...
export async function POST(request: NextRequest) {
    try {
        ...
        if (idempotencyKey) {
            const { exists, result } = await IdempotencyHandler.checkIdempotency(
                idempotencyKey,
                user.id
            );
        ...
        if (rpcError) {
            ...
            return NextResponse.json(
                { error: 'Failed to create selection', details: rpcError.message },
                { status: 500 }
            );
        }
        ...
        if (idempotencyKey) {
            await IdempotencyHandler.storeResult(
                idempotencyKey,
                user.id,
                responseData
            );
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (rpcError) {
            ...
            return NextResponse.json(
                { error: 'Failed to fetch selections', details: rpcError.message },
                { status: 500 }
            );
        }
