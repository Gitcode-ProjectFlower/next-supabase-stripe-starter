...
export async function Settings({ searchParams }: SettingsProps) {
  ...
  const [session, subscriptionData, productsData] = await Promise.all([
    getSession(),
    getSubscription(shouldCheckStripe),
    getProducts(),
  ]);

  const subscription = subscriptionData as SubscriptionWithProduct | null;
  const products = productsData as ProductWithPrices[];
  ...
  return (
    <>
      ...
      <Tabs defaultValue={defaultTab} className='grid w-full grid-cols-12 gap-6'>
        ...
        <div className='col-span-12 space-y-6 md:col-span-9'>
          ...
          {/* Language Tab */}
          <TabsContent value='language' className='mt-0 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm'>
            <LanguageSection />
          </TabsContent>
          ...
          {/* Notifications Tab */}
          <TabsContent value='notifications' className='mt-0 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm'>
