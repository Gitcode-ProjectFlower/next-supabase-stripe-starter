...
function updateAncestorsFor(nodes: TreeNode[], selected: Set<NodeId>, id: NodeId) {
...
function flattenVisible(
  ...
  const dfs = (list: TreeNode[], depth: number) => {
    for (const n of list) {
      ...
      const isOpen = expanded.has(n.id) || (q !== '' && (n.children?.some(keep) ?? false));
...
export function TreeMultiSelect({
  ...
  const rows = useMemo(() => flattenVisible(data, expanded, search), [data, expanded, search]);
  ...
  const selectedLabels = useMemo(() => {
    ...
    const collect = (list: TreeNode[]) => {
      for (const n of list) {
        ...
        if (selected.has(n.id) && (!n.children || n.children.length === 0)) labels.push(n.name);
  ...
  return (
    <div className={cn('relative', className)} ref={wrapperRef}>
      ...
      >
        <span
          className={cn(
            'min-w-0 flex-1 overflow-x-auto whitespace-nowrap pr-2',
            selectedLabels ? 'font-medium text-gray-900' : 'text-muted-foreground'
          )}
        >
      ...
      {open && (
        ...
        >
          ...
          <div ref={containerRef} className='max-h-[300px] overflow-x-auto overflow-y-auto py-1'>
              ...
              {rowVirtualizer.getVirtualItems().map((vi) => {
                ...
                return (
                  ...
                  >
                    ...
                    {hasKids ? (
                      <button
                        className='flex h-6 w-6 shrink-0 items-center justify-center rounded-sm text-gray-500 hover:bg-gray-100'
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleExpand(id);
                        }}
                    ...
                    <div className='flex flex-1 cursor-pointer items-center gap-2 py-1' onClick={() => toggleCheck(id)}>
                      <TriCheckbox state={st} />
                      <span className='select-none truncate text-sm'>{node.name}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className='flex items-center justify-between border-t bg-white p-2'>
            ...
            <span className='px-2 text-xs text-accent-foreground'>Selected: {selected.size}</span>
          </div>
        </div>
      )}
    </div>
