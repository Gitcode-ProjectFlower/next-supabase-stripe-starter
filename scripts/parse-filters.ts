...
function buildTree(rows: CSVRow[], levelKeys: string[]): TreeNode[] {
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < levelKeys.length; i++) {
      ...
      const nodeId = parentId ? `${parentId}-${slugify(value)}` : slugify(value);
...
function generateTypeScriptFile(tree: TreeNode[], fileName: string, exportName: string): void {
...
async function main() {
  ...
  const sectorsTree = buildTree(rows, ['sector_level1', 'sector_level2', 'sector_level3']);
  console.log(`   Generated ${sectorsTree.length} top-level sectors`);

  console.log('\n🗺️  Building Regions tree...');
  const regionsTree = buildTree(rows, ['region_level1', 'region_level2', 'region_level3', 'region_level4']);
